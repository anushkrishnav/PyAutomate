...
def send_message(json: str) -> None:
    answer = requests.post(settings.DISCORD_WEBHOOK_URL, data=json, headers={
...
def parser():
    ...
    if action == "opened":
        if type == "issues":
            send_issue(envelope["sender"]["avatar_url"], envelope["sender"]
                       ["login"], envelope["issue"]["title"], envelope["issue"]["url"])
        elif type == "pull_request":
            send_pr(envelope["sender"]["avatar_url"], envelope["sender"]["login"],
                    envelope["pull_request"]["title"], envelope["pull_request"]["url"])
